...
class sedge_sequence(torch.nn.Module):
    def __init__(self, *, input_size=1, hidden_sizes=[], output_sizes=[], ZeroOrderHoldRegularization=[],
                 input_bias=True, bias_init='zero',
                 output_bias=True, complex_output=False,
                 norm=False, norm_type='bn', B_C_init='orthogonal', stability='relu', trainable_SkipLayer=True, act='LeakyRELu',dropout=0.0, **kwargs):
        super(sedge_sequence, self).__init__()
        self.input_size = input_size # nb of channels of the spectrogram (batch_dim, sequence_length, input_size)
    ...
    def forward(self, inputs):
        ...
        # Since UMX is designed with time at first dimension, 
        # but s-edge layers ask for batch first, we apply transposition twice
