...
# ---------------------------------------------------------
n_hop = N//10  # Votre pas de recouvrement (ex: 256 pour un overlap de 75%)
...
ax1.plot(t, w_base, label=r'Hann + $\varepsilon$', color='blue', alpha=0.7)
ax1.plot(t, w_reg, label='Proposed window', color='red', linestyle='--')
...
ax2.plot(freqs, W_hann_mag, label='Hann', color='green', linestyle='--')
...
plt.savefig('/home/adubois/openunmix/OpenUnmix/fig/window_combined.png', bbox_inches='tight')
plt.close()


# 2. Figures individuelles (conservées pour compatibilité ou autre usage)
# 2.1 Domaine temporel individuel
fig1 = plt.figure(figsize=(11, 6))
plt.scatter(t[::10], w_hann[::10], label='Hann standard (no epsilon)', color='green', marker = "x", linewidths=2)
plt.plot(t, w_base, label='Hann + 1e-4', color='blue', alpha = 0.7)
plt.plot(t, w_reg, label='Proposed window', color='red', linestyle='--')
plt.xlabel('Samples')
plt.ylabel('Amplitude', labelpad=25)
plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.24), ncol=3, frameon=True)
plt.grid(True)
fig1.subplots_adjust(left=0.15, right=0.95, top=0.92, bottom=0.28)
plt.savefig('/home/adubois/openunmix/OpenUnmix/fig/window_time_domain.png')
plt.close()

# 2.2 Vérification visuelle du profil OLA
fig2 = plt.figure(figsize=(11, 6))
plt.plot(ola_buffer, color='purple', label=r'$\sum_m w_{\mathrm{reg}}^2(n - mH)$')
plt.axvline(2*N, color='gray', linestyle=':', label='Stable zone')
plt.axvline(3*N, color='gray', linestyle=':')
plt.xlabel('Assembled Signal Samples')
plt.ylabel('Denominator Amplitude')
plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.24), ncol=2, frameon=True)
plt.grid(True)
fig2.subplots_adjust(left=0.15, right=0.95, top=0.92, bottom=0.28)
plt.savefig('/home/adubois/openunmix/OpenUnmix/fig/window_ola_verification.png')
plt.close()

# 2.3 Réponse fréquentielle individuelle
fig3 = plt.figure(figsize=(11, 6))
plt.plot(freqs, W_hann_mag, label='Hann standard (no epsilon)', color='green', linestyle='--')
plt.plot(freqs, W_base_mag, label='Hann + 1e-4', color='blue', alpha=0.5)
plt.plot(freqs, W_reg_mag, label='Proposed window', color='red', linestyle='--')
plt.xlabel('Normalized Frequency')
plt.ylabel('Magnitude (dB)', labelpad=12)
plt.ylim([-180, max(np.max(W_hann_mag), np.max(W_base_mag), np.max(W_reg_mag)) + 10])
plt.xlim(-0.25,0.25)
plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.24), ncol=3, frameon=True)
plt.grid(True)
fig3.subplots_adjust(left=0.15, right=0.95, top=0.92, bottom=0.28)
plt.savefig('/home/adubois/openunmix/OpenUnmix/fig/window_frequency_response.png')
plt.close()