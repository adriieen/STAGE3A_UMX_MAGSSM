...
class Sedge_OpenUnmix(nn.Module):
    ...
    def __init__(
        ...
        super(Sedge_OpenUnmix, self).__init__()
        ...
        if output_size_factors == None :
            output_sizes_factors = np.array([1 for _ in range(int(nb_layers))])

        else : output_sizes_factors = np.array(output_size_factors)

        self.sedge = sedge_sequence(
            input_size=hidden_size,
            hidden_sizes = list((hidden_size // hidden_size_factors).astype(int)),
            output_sizes = list((hidden_size // output_sizes_factors).astype(int)),
            dropout=0.4
        )
        ...
        #     output_sizes = [hidden_size, hidden_size, hidden_size],
        #     dropout=0.4
        # )
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        print("input forward pass", x.data.shape)
        ...
        print("input sedge", x.data.shape)
