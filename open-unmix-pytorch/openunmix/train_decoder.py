...
def train(args, trainable_decoder, encoder, device,
          train_sampler, optimizer, scaler=None, 
          alpha = 1e-2, beta = 0
          ):
...
def valid(args, trainable_decoder, encoder, device, 
        valid_sampler, use_amp=False, 
        alpha = 1e-2, beta = 0,
        ):
    losses = utils.AverageMeter()
    ...
    with torch.no_grad():
        ...
        return (losses.avg if losses.count > 0 else float('nan')), X, Y_hat



def main():
    parser = argparse.ArgumentParser(description="Open trainable_decoder Trainer")
    ...
    if args.model: # fine tune model
        ...
        print("Proceeding by chunks: ", trainable_decoder.magssm_decoder.single_sequence_decoder.mimo.progressive)
    ...
    optimizer = torch.optim.AdamW(trainable_decoder.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    ...
    stft_best_path   = Path(target_path, args.target + "_stft_best.png")  
    trainable_decoder_best_path = Path(target_path, args.target + "_magssm_best.png") 

    for epoch in t:
        t.set_description("Training epoch")
        end = time.time()
        train_loss = train(args, trainable_decoder, encoder, device, train_sampler, optimizer, scaler=scaler,  
                           alpha=args.alpha, beta = args.beta)
        
        valid_loss, X_val, Y_hat_val = valid(args, trainable_decoder, encoder, device, 
                                             valid_sampler, use_amp=args.amp, 
                                             alpha=args.alpha, beta = args.beta)
        ...
        utils.save_checkpoint(
            {
                "epoch": epoch + 1,
                "state_dict": trainable_decoder.state_dict(),
                "best_loss": es.best,
