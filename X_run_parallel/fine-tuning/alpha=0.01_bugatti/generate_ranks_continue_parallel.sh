#!/usr/bin/env bash
# ============================================================================
#  GÉNÉRATEUR DE LANCEURS DDP SWEEP POUR CONTINUER LE FINE-TUNING
#  Usage : bash generate_ranks_continue_parallel.sh
# ============================================================================

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/config_continue_training.sh"
...
for RANK in $(seq 0 $((NNODES - 1))); do
    ...
    if [ "$RANK" -eq 0 ]; then
        OUTFILE="$SCRIPT_DIR/rank0_${NAME}_continue_parallel.sh"
    else
        OUTFILE="$SCRIPT_DIR/rank${RANK}_continue_parallel.sh"
    fi

    # Commencer à écrire le script
    cat > "$OUTFILE" <<HEREDOC
#!/usr/bin/env bash
# Script de continuation DDP Fine-Tuning pour le noeud $RANK ($NAME)
...
echo "============================================================"
echo "  Reprise du sweep Fine-Tuning DDP — Noeud $RANK ($NAME)"
...
echo "============================================================"
echo "  Continuation Fine-Tuning DDP terminée sur le noeud $RANK !"
echo "============================================================"
HEREDOC
...
echo ""
echo "Tous les scripts de reprise de sweep sont à jour. Config utilisée :"
