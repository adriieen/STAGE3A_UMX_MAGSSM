...
def load_target_models(targets, model_str_or_path="umxl", device="cpu", pretrained=True, magssm=False):
    """Core model loader
    ...
    if not model_path.exists():
        ...
        for target in targets:
            ...
            if not magssm: # classic model with STFT encoder

                models[target] = sedge_umx.Sedge_OpenUnmix(
            ...
            else : # encoder is a trainable spectrogram:
...
def load_separator(
    ...
    if model_path.exists():
        ...
        target_models = load_target_models(targets=targets, model_str_or_path=model_path, pretrained=pretrained, magssm = magssm)
        ...
        if not magssm:
            ...
            separator = sedge_mask.Separator(
                target_models=target_models,
                niter=niter,
                residual=residual,
                wiener_win_len=wiener_win_len,
