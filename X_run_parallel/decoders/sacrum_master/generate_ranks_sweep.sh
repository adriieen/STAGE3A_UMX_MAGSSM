#!/usr/bin/env bash
# ============================================================
#  GÉNÉRATEUR SWEEP DECODER — recrée tous les rankN_sweep.sh à partir de config_sweep.sh
...
for RANK in $(seq 0 $((NNODES - 1))); do
    ...
    cat > "$OUTFILE" <<HEREDOC
#!/usr/bin/env bash
# Script de sweep DDP pour le noeud $RANK ($NAME) - Decoder Training
...
echo "============================================================"
echo "  Début du sweep multi-machines — Decoder — Noeud $RANK ($NAME)"
...
HEREDOC
    ...
    [ "${FLAG_FFT_KERNEL:-0}" -eq 1 ] && echo "    --fft_kernel \\" >> "$OUTFILE"
    ...
    [ "$FLAG_PHASE_CORRECTION" -eq 1 ] && echo "    --phase-correction \\" >> "$OUTFILE"
...
echo ""
echo "Tous les scripts de sweep decoder sont à jour. Config utilisée :"
